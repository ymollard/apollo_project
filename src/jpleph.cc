...
static void interp( struct interpolation_info *iinfo,
        const double coef[], const double t[2], const int ncf, const int ncm,
        const int na, const int velocity_flag, double posvel[])
{
   const double dna = (double)na;
   const double temp = dna * t[0];
   const int l = (int)temp;
   double vfac, unused_temp1;
   const double tc = 2.0 * modf( temp, &unused_temp1) - 1.0;
   int i, j;
...
/*   read correct record if not in core (static vector buf[])   */
   ...
   if( eph->pvsun_t != et)   /* If several calls are made for the same et, */
      ...
      recompute_pvsun = 1;   /* the first run through.                     */
      eph->pvsun_t = et;
      }
   else
      recompute_pvsun = 0;
...
double jpl_get_constant( const int idx, void *ephem, char *constant_name)
{
   struct jpl_eph_data *eph = (struct jpl_eph_data *)ephem;
   double rval = 0.;

   *constant_name = '\0';
   if( idx >= 0 && idx < (int)eph->ncon)   /* not a valid index */
      {
      fseek( eph->ifile, 84L * 3L + (long)idx * 6, SEEK_SET);
...
int main() {
	...
	void* e = jpl_init_ephemeris("/home/ymollard/ctadev/lib/JPLEPH", NULL, NULL);
	r = jpl_pleph(e, 2441159.187535, 10, 3, rrd, 0);
	double x = rrd[0];
	double y = rrd[1];
	double z = rrd[2];

	double alpha_moon = atan2(y, x);
   	double delta_moon = asin(z/sqrt(x*x+y*y+z*z));
	cout << alpha_moon*180/3.141592 << endl << delta_moon*180/3.141592 << endl;
